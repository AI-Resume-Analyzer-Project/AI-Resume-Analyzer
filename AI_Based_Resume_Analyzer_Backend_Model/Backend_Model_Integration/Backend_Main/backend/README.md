...
- ATS score is calculated by backend logic: `(matched JD skills / total JD skills) * 100`.
- AI is optional and used only for suggestions. Rule-based recommendations work without an API key.
...
Create a `.env` file from `.env.example`, then create the database:
...
AI never calculates scores, updates data, or ranks resumes. The backend remains the source of truth.
