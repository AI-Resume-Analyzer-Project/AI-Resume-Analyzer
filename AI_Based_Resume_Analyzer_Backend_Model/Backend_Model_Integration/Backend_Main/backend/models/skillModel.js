...
const findOrCreate = async (skillName) => {
  ...
  const [rows] = await db.execute('SELECT skill_id, skill_name FROM skills WHERE skill_name = ?', [name])
  return rows[0]
}
...
const list = async ({ limit, offset }) => {
  const [records] = await db.execute('SELECT * FROM skills ORDER BY skill_name LIMIT ? OFFSET ?', [limit, offset])
...
module.exports = { create, findOrCreate, findManyByNames, list, byResume, byJd }
