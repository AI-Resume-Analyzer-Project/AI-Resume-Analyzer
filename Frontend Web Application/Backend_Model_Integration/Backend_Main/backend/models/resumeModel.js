...
const list = async ({ limit, offset, user }) => {
  ...
  const [records] = await db.query(`SELECT resume_id, candidate_id, uploaded_by, file_name, original_name, upload_date FROM resumes ${where} ORDER BY resume_id DESC LIMIT ? OFFSET ?`, [...values, limit, offset])
