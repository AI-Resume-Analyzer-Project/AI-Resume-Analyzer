...
exports.upload = asyncHandler(async (req, res) => {
  ...
  const jobDescription = req.body.jd_text || req.body.job_description || req.query.jd_text || ""

  console.log("Extracted JD Text:", jobDescription.substring(0, 100) || "(empty)");
  ...
  await logService.write(req, 'resume_upload', `Resume ${resume.resume_id} uploaded`)
...
exports.remove = asyncHandler(async (req, res) => {
  ...
  if (!resume) throw new ApiError(404, 'Resume not found')
  analysisService.ensureOwnership(resume, req.user, 'resume')
  await resumeModel.remove(req.params.id)
  await logService.write(req, 'resume_deleted', `Resume ${req.params.id} deleted`)
