...
const tokenFor = (user) => jwt.sign({ user_id: user.user_id, email: user.email, role: user.role }, config.jwtSecret, { expiresIn: config.jwtExpiresIn })

exports.signup = asyncHandler(async (req, res) => {
  if (req.body.role === 'admin') throw new ApiError(400, 'Admin role cannot be assigned during signup. Please register as candidate or recruiter.')
  const existing = await userModel.findByEmail(req.body.email)
  if (existing) throw new ApiError(409, 'Email is already registered')
  const password = await bcrypt.hash(req.body.password, 10)
  const role = req.body.role || 'candidate'
  const userId = await userModel.create({ name: req.body.name, email: req.body.email, password, role })
...
exports.signin = asyncHandler(async (req, res) => {
  ...
  success(res, 'Signin successful', { user: publicUser, token: tokenFor(publicUser) })
})

exports.profile = asyncHandler(async (req, res) => {
  success(res, 'Profile fetched successfully', await userModel.findById(req.user.user_id))
})
...
exports.updateProfile = asyncHandler(async (req, res) => {
  ...
  success(res, 'Profile updated successfully', await userModel.findById(req.user.user_id))
})
...
exports.remove = asyncHandler(async (req, res) => {
  ...
  await logService.write(req, 'user_deleted', `Admin deleted user ${req.params.id}`)
